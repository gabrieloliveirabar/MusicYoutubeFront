...
export interface ISongContext {
  ...
  mp3_Song: any;
  setMp3_Song: Dispatch<SetStateAction<undefined>>;
...
export const SongProvider = ({ children }: ISongProvider) => {
  ...
  const SongPost = async (link: string) => {
    ...
    await api
      ...
      .catch((err) => {
        setOpenModalErrorMsg(true);
        setModalSongOpen(false);
      });
  };

  useEffect(() => {
    api
      .get(`http://localhost:8000/api${mp3_file_url}`, {
